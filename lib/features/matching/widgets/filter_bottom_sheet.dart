...
class _FilterSheetState extends ConsumerState<_FilterSheet> {
  ...
  void initState() {
    ...
    _reliabilityScore = f.minReliability;
    _selectedCourse = f.course;
    _selectedGrade = f.targetGrade;
    _selectedStyle = f.learningStyle;
  }
  ...
  void _apply() {
    ref.read(filterProvider.notifier).update(FilterModel(
      course: _selectedCourse,
      minReliability: _reliabilityScore,
        ...
        children: [
          ...
          Row(
            ...
            children: [
              TextButton(onPressed: _reset,
                  child: const Text('Reset', style: TextStyle(color: AppColors.textSecondary, fontSize: 14))),
              const Text('Filter Matches', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold, color: Colors.black87)),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: AppColors.backgroundBlue, borderRadius: BorderRadius.circular(12),
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                items: courses.map((c) => DropdownMenuItem(value: c, child: Text(c, style: const TextStyle(fontSize: 13)))).toList(),
          ...
          Row(children: [
            ...
            Container(
              ...
              child: Text('${_reliabilityScore.round()}%', style: const TextStyle(fontSize: 12, color: Colors.white, fontWeight: FontWeight.w700)),
            ),
          ]),
          SliderTheme(
            ...
            child: Slider(value: _reliabilityScore, min: 0, max: 100, divisions: 20,
                onChanged: (val) => setState(() => _reliabilityScore = val)),
          ),
          Text('Hide partners who frequently miss sessions.', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
          const SizedBox(height: 20),

          // Target Grade
          const Text('Target Grade', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.black87)),
          const SizedBox(height: 10),
          Row(children: _grades.map((grade) {
            final selected = _selectedGrade == grade;
            return Padding(
              padding: const EdgeInsets.only(right: 10),
              child: GestureDetector(
                onTap: () => setState(() => _selectedGrade = selected ? null : grade),
                child: Container(
                  ...
                  child: Text(grade, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600,
          ...
          const SizedBox(height: 10),
          Row(children: _styles.map((style) {
            final selected = _selectedStyle == style;
            return Padding(
              padding: const EdgeInsets.only(right: 10),
              child: GestureDetector(
                onTap: () => setState(() => _selectedStyle = selected ? null : style),
                child: Container(
                  ...
                  child: Text(style, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600,
                      color: selected ? Colors.white : Colors.black54)),
                ),
              ),
            );
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: const Text('Apply Filters', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white)),
            ),
          ),
        ],
      ),
